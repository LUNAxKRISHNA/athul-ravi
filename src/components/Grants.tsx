  ...
  return (
    <Section id="grants" eyebrow="Funding" title="Grants & Funded Research">
      <ul className="space-y-5">
        {grants.map((g) => (
          <li key={g.title} className="flex flex-col gap-1 border-b border-line pb-5 last:border-0">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <h4 className="font-heading text-base font-medium text-ink">{g.title}</h4>
              <span className="text-xs font-medium uppercase tracking-wide text-accent">{g.year}</span>
            </div>
